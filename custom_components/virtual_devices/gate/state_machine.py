...
class GateStateMachine:
    ...
    def _asymmetric_command(
        self, snapshot: GateSnapshot, command: GateCommand
    ) -> GateTransition:
        """Apply the fixed asymmetric single-source controller profile."""
        if (
            command is GateCommand.OPEN
            and snapshot.state in (GateState.OPEN, GateState.OPENING)
        ) or (
        ...
        if (snapshot.state is GateState.CLOSED and command is GateCommand.OPEN) or (
            ...
            pulse_count = 1
        elif snapshot.state is GateState.OPENING and command is GateCommand.CLOSE:
            pulse_count = 2
        elif snapshot.state is GateState.CLOSING and command is GateCommand.OPEN:
            pulse_count = 1
        elif (
            snapshot.state is GateState.STOPPED
            and snapshot.last_direction is GateDirection.OPENING
        ):
            pulse_count = 2 if command is GateCommand.OPEN else 1
        else:
            return GateTransition(snapshot)
